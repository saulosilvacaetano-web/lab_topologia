...
```bash
sudo docker exec clab-saulo-lab-node-b apk add bpftool
```
...
sudo docker exec clab-saulo-lab-node-b rm -f /sys/fs/bpf/xdp_test

# Carregar e pinar o programa no filesystem BPF
sudo docker exec clab-saulo-lab-node-b \
...
```bash
sudo docker exec clab-saulo-lab-node-a ping -c 5 10.0.0.2
```
...
```bash
sudo docker exec clab-saulo-lab-node-b bpftool map dump name packet_count_ma
```
...
```bash
sudo docker exec clab-saulo-lab-node-b ip link set dev eth1 xdpgeneric off
```

Verifique que a conectividade foi restaurada:

```bash
sudo docker exec clab-saulo-lab-node-a ping -c 3 10.0.0.2
```
...
```bash
sudo containerlab destroy -t lab-saulo.clab.yml
```
...
```
saulo-lab/
├── lab-saulo.clab.yml        # Definição da topologia Containerlab
...
└── clab-saulo-lab/           # Arquivos de runtime gerados pelo Containerlab
